...
for epoch in range(num_epochs):
    ...
    for step, batch in enumerate(train_dl):
        completed_steps += 1
        accelerator.print(completed_steps)
        ...
        if step % args.save_checkpoint_steps == 0:
            ...
            log_metrics(step, {"elapsed_time": elapsed_time,
            ...
            save_dir = os.path.join(os.path.join(args.save_dir, run_name), f"step_{completed_steps}")
...
accelerator.print('[INFO] evaluating and saving model after training...')
roc, precMi, precW, recMi, recW, f1Mi, f1W, eval_loss = evaluate_fn(args) 
log_metrics(step, {"loss/train": loss.item(),
    "loss/eval": eval_loss.item(),
    "lr": lr,
    # "accuracy": acc,
    "roc_auc_score": roc,
...
accelerator.wait_for_everyone()
unwrapped_model = accelerator.unwrap_model(model)
save_dir = os.path.join(os.path.join(args.save_dir, run_name), f"step_final")
#accelerator.save_state(save_dir)
if accelerator.is_main_process:
    unwrapped_model.save_pretrained(save_dir, save_function=accelerator.save)
    if args.push_to_hub:
